	...
	class functions extends Database
    {
		...
		public function searchToursheaders($data){
			$where = $data['search_header'];
			 $query="select * from tours where (city LIKE '%$where%' OR nameof_tour LIKE '%$where%' OR shortdescription LIKE '%$where%' OR longdescription LIKE '%$where%' OR states LIKE '%$where%') AND show_online='Yes'";
			// echo $query="select * from tours where city LIKE '%$where%' OR nameof_tour LIKE '%$where%' OR shortdescription LIKE '%$where%' OR longdescription LIKE '%$where%' OR states LIKE '%$where%' AND show_online='Yes'  AND from_date BETWEEN '$from_date' AND '$to_date'";
			 return $this->query($query);
		}
		...
		//======STATIC URLS BEGINS=======
		function getStaticUrl($tour_id, $tour_details){
			//$tour = str_replace('\\','-',str_replace('/','-',str_replace('&', '-', $tour_type)));
			/*$tour_id = str_replace('\\','-',str_replace('/','-',str_replace('&', '-', $tour_id)));
			$tour_details = str_replace(' ', '-', str_replace('\\', '-', str_replace('/', '-', str_replace('&', '-', $tour_details))));*/

			
			//$tour_id = str_replace(':','-', str_replace('.','-', str_replace('\\','-',str_replace('/','-',str_replace('&', '-', $tour_id)))));
			//$tour_details = str_replace(':','-', str_replace('.','-', str_replace(' ', '-', str_replace('\\', '-', str_replace('/', '-', str_replace('&', '-', $tour_details))))));
			...
			$replace_keywords = array("---", "--",
									  ...
									  "&", "&", "&", "&", "&", "&", "&", 
									  " ");
			$tour_id = str_replace($replace_keywords, '-', $tour_id);
			$tour_details = str_replace($replace_keywords, '-', $tour_details);

			$url = $tour_id.'-'.$tour_details;
			//return "http://www.seeinindia.com/".$url;
			return $url;
		}
		//======STATIC URLS ENDS=======
	} 
